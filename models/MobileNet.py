...
# 定义卷积层，使用1D卷积
Conv1 = {'in_channel': 1, 'filters': 64, 'kernel': 3, 'stride': 1, 'dilation': 1, 'padding': 1}
Conv2 = {'in_channel': 64, 'filters': 128, 'kernel': 3, 'stride': 1, 'dilation': 1, 'padding': 1}
m1 = MobileNetBlock( Conv1, BN1, Pooling1, Dropout1, block_type)
m2 = MobileNetBlock( Conv1, BN1, Pooling1, Dropout1, block_type)
...
class MobileNet(nn.Module):
    def __init__(self):
        ...
        self.cnn = nn.Sequential(
            ...
            MobileNetBlock(mobilenet_dict=Conv1, bn_dict=BN1, pooling_dict=Pooling1, dropout_dict=Dropout1,
                       block_type=block_type),
        )
        ## 32*128*144
        # 共享特征提取后的线性层部分
        linear1_in_dim = 128 * 144  # 计算卷积层输出后的维度
