...
class CNN(nn.Module):
    def __init__(self):
        ...
        # 定义卷积层，使用1D卷积
        Conv1 = {'in_channel': 32, 'filters': 64, 'kernel': 3, 'stride': 1, 'dilation': 1, 'padding': 1}
        ...
        # 共享特征提取后的线性层部分
        linear1_in_dim = 128 * 8  # 计算卷积层输出后的维度
        ...
        # 分类任务的输出：4类
        Linear2_class = {'in_dim': 64, 'out_dim': 4}  # 4类分类
        ...
        # 回归任务的输出：预测16个数据点
        Linear2_regression = {'in_dim': 64, 'out_dim': 16}  # 回归预测未来16个数据
    ...
    def forward(self, data):
        ...
        # 回归任务的输出
        reg_out = self.regression_branch(cnn_out)  # [batch_size, 16]

        return class_out, reg_out

    def get_model_name(self):
        return 'CNN-Dual-Task'