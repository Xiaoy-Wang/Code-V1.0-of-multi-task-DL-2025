...
def calculate_class_weights_torch(labels, num_classes=None):
    ...
    # 首先将 numpy 数组转换为 torch tensor
    labels_tensor = torch.tensor(labels)

    # 然后调用 long() 方法将其转换为 int64 类型
    class_counts = torch.bincount(labels_tensor.long(), minlength=num_classes).float()
...
def calculate_class_weights(labels, num_classes=None, verbose=True):
    ...
    if num_classes is None:
        classes = np.unique(labels)  # 获取所有唯一的类别
    else:
        classes = np.arange(num_classes)  # 如果已知类别数量，可以直接生成类别标签
