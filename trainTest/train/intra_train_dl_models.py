...
def train_test_intra_model(settings_dict, model, train_loader, valid_loader, test_loader, device, save_path, callbacks,
    ...
    for e in range(1, 1 + callbacks['epoch']):
        ...
        for data, target in train_loader:
            ...
            phase_pred,angle_pred = model(data,label)
            loss_phase = nn.CrossEntropyLoss(phase_pred, target[:, 0])
            loss_angle = nn.MSELoss(angle_pred.squeeze(), target[:, 1:])
            # loss = criterion(predict, target)
            loss = loss_phase+loss_angle
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for data, target in valid_loader:
                ...
                target = target.to(device=device).view(-1)
                # 获取模型的输出
                # logits, predict = model(data)  # 假设返回的是元组
                # # 计算损失
                # loss = criterion(logits, target)  # 使用 logits 计算损失
                predict = model(data,label)
                loss = criterion(predict, target)
                _, predict_index = torch.max(predict.detach().to(device='cpu'), dim=1)
