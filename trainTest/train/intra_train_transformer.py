...
def trainModel(path,current_exp_time,model, Epoch, optimizer, trainLoader, testLoader,validLoader,EarlyStop=False):
    ...
    with torch.no_grad():
        ...
        for data, label1, label2 in testLoader:
            ...
            preResult_flat = preResult.detach().view(-1).to(device='cpu').numpy()
            tempR2Score += r2_score(label1_flat, preResult_flat)
            tempRMSE += np.sqrt(mean_squared_error(label1_flat, preResult_flat))
    ...
    train_results_save_path =os.path.join(path, f"train_results_{current_exp_time}.csv")
    valid_results_save_path = os.path.join(path, f"valid_results_{current_exp_time}.csv")
...
def trainModel_CNN(path,current_exp_time,model, Epoch, optimizer, trainLoader, testLoader,validLoader,EarlyStop=False):
    ...
    valid_results = pd.DataFrame({
        'train R² Score': validR2ScoreEpoch,
        'valid MSE Loss': validRmseLossEpoch,
        'valid Cross Entropy Loss': validCrossLossEpoch,
        'valid Accuracy': validAccuracyEpoch,
        'valid R² Score': validR2ScoreEpoch
    ...
    train_results_save_path =os.path.join(path, f"train_results_{current_exp_time}.csv")
    valid_results_save_path = os.path.join(path, f"valid_metrics_{current_exp_time}.csv")
